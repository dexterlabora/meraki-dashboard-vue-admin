...
module.exports = {
  ...
  watch: {
    ...
    ssidForm: function () {
      /*
      console.log('ssidForm', this.ssidForm);
      console.log('ssid', this.ssid);
      if(!Object.is(this.ssidForm, this.ssid)){
      //if(this.ssidForm !== this.ssid){
        this.formUpdated();
      }
      */
    }
  ...
  methods: {
    ...
    updateSsid: function ($index) {
      ...
      if (this.ssid) {
        ...
        axios.put('/api/networks/'+this.net.id+'/ssids/'+this.ssidForm.number, this.ssidForm)
          .then(res => {
            //this.$set('ssids', ssids);
            //this.ssids.push(this.ssidForm);
            console.log('updateSsid res.data',res.data );
            //this.ssids[this.ssidForm.number] = res.data; // update local copy of SSIDs
            //this.ssid = this.ssids[this.ssidForm.number]; // update form with new version
            ...
            this.notify("error","Bummer, Failed to save changes. \n /api/networks/"+this.net.id+"/ssids/"+this.ssidForm.number);
          });
